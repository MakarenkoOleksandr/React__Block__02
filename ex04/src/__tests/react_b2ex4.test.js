...
describe("b2ex04: Display categories ( Functional )", () => {
  it("#1 App.js must render functional component List", () => {
    ...
    const key3 = Object.keys(
      view[key][key2].alternate.child.child.stateNode
    )[1];
    const key4 = Object.keys(
      view[key][key2].alternate.child.child.stateNode[key3]
    )[0];
    expect(
      `${view[key][key2].alternate.child.child.stateNode[key3][key4].type}`
        .slice(0, 50)
        .includes("class")
    ).toBe(false);
  ...
  it("#2 List must render functional component Item", () => {
    ...
    expect(
      `${view[key][key2].alternate.child.child.stateNode[key3][key4][0].type}`
        .slice(0, 50)
        .includes("class")
    ).toBe(false);
  ...
  it("#3 Pass categories array from App.js to List.js", () => {
    ...
    expect(
      view[key][key2].alternate.child.child.stateNode[key3][key4].props
        .categories.length > 0
    ).toBe(true);
  });
